/**	@file DirectX.xpp
*	Contains implementation for WinMain and methods to create a window in DirectX
*/
#include "pch.h"
...
IDXGISwapChain1* mSwapChain; //for double buffering
...
*	Creates an empty on the screen
*/
int WINAPI WinMain(HINSTANCE instance, HINSTANCE previousInstance, LPSTR commandLine, int showCommand)
{
	std::wstring windowClassName = L"RenderingClass"; //to identify the window

	InitializeWindow(instance, windowClassName, L"FIEA Game Engine - DirectX", showCommand);
	InitializeDirectX();
	...
	while (message.message != WM_QUIT)
	{
		...
		else
		{
			...
			// Game-specific draw and update code goes here
...
void InitializeWindow(HINSTANCE instance, const std::wstring& className, const std::wstring windowTitle, int showCommand)
{
	ZeroMemory(&mWindow, sizeof(mWindow)); //init every member of the struct to 0
...
void InitializeDirectX()
{
	...
	//list of all DirectX versions supported, from highest to lowest
